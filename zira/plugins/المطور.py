...
ZelzalDV_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗭𝗘𝗗𝗧𝗵𝗼𝗻 - اوامــر المطـور المســاعد](t.me/OE_40) 𓆪\n\n"
...
async def _(event):
    ...
    result = "**•❐• قائمــة مـطـورين البــوت الخـاص بـك مـن max :**\n\n"
...
async def _(event):
    ...
    output += "**✾╎يتم الان اعـادة تشغيـل بـوت ماكس انتظـر 2-1 دقيقـه ▬▭ ...**"
    msg = await edit_or_reply(event, output)
    await event.client.reload(msg)
...
async def _(event):  # sourcery no-metrics
    ...
    output = (
        result + "**✾╎يتم الان اعـادة تشغيـل بـوت ماكس انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
    msg = await edit_or_reply(zedevent, output)
